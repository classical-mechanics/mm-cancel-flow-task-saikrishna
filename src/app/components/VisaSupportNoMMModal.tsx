// components/VisaSupportNoMMModal.tsx
"use client";
...
export type VisaSupportNoMMResult = {
  employerProvidesLawyer: boolean;
  visaType: string;
};

export default function VisaSupportNoMMModal({
  open,
  onClose,
  onBack,
  onComplete,
  ...
  onClose: () => void;
  onBack: () => void;
  onComplete: (result: VisaSupportNoMMResult) => void;
}) {
    ...
    >
      ...
      <section className="relative z-10 h-dvh w-full overflow-y-auto bg-white rounded-none md:h-auto md:w-[min(1000px,92vw)] md:rounded-2xl md:border md:border-black/10 md:card-shadow">
        ...
        <div className="px-4 pb-6 pt-4 md:grid md:grid-cols-2 md:gap-8 md:px-6 md:py-6">
          {/* Left: copy + form */}
          <div>
            <h1 id="nomm-title" className="text-2xl font-semibold tracking-tight md:text-[28px]">
              You landed the job!{" "}
              <span className="italic font-semibold">That’s what we live for.</span>
            </h1>

            <p className="mt-3 max-w-prose text-sm leading-6 text-neutral-700">
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              {hideOpposite && (
                <button
                  ...
                  onClick={() => { setAnswer(null); setVisa(""); }}
                >
                  Change
                </button>
              )}
            ...
            {/* Radios — hide opposite after selection */}
            ...
            <div className="mt-4">
              ...
              <input
                ...
                className="h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-neutral-900 disabled:opacity-50"
                disabled={!answer}
              />
            </div>
            ...
            <button
              ...
              onClick={() =>
                onComplete({
                  employerProvidesLawyer: answer === "yes",
                  visaType: visa.trim(),
                })
              }
              className={[
